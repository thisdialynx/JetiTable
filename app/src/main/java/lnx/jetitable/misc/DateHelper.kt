...
class DateHelper @Inject constructor() {
    ...
    private val currentAcademicYear =
        if (getSemester() == SemesterType.AUTUMN) academicYearsInt[0]..academicYearsInt[0] else academicYearsInt[1]..academicYearsInt[1]

    val selectedDateInfo = selectedDate.map {
        DateState(
    ...
    fun getSemester(): SemesterType {
        return if (selectedMonth in 7..11) SemesterType.AUTUMN else SemesterType.SPRING
    }
