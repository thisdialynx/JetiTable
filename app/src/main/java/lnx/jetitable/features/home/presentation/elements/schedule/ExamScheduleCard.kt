...
fun ExamScheduleCard(
    examList: ScheduleResult<List<ExamNetworkData>>
) {
                ...
                when (list) {
                    is ScheduleResult.Success -> {
                        if (list.data.isEmpty()) {
                            ...
                            list.data.forEachIndexed { index, examItem ->
                                ContentRow(
                                    index = index,
                    ...
                    is ScheduleResult.Failure -> {
                        val reason = when (list.error) {
                            ScheduleFetchFailureReason.EMPTY -> R.string.no_exams to Info
                            ...
                            ScheduleFetchFailureReason.IO_ERROR -> R.string.something_went_wrong to Error
